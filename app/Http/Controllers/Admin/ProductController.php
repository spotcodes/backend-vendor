...
class ProductController extends Controller
{
    ...
    public function manage_product_process(Request $request)
    {
        ...
        if($request->post('id')>0){
           ...
           $model=Product::whereRaw('id="'.$request->post('id').'" '.$condition.'')->first();
            $msg="Product updated";
        }else{
            $model=new Product();
        ...
        $model->status=1;
        $model->added_by=session()->get('ADMIN_ID');
      // prx($model);
        $model->save();
        $pid=$model->id;
        /*Product Attr Start*/ 
    ...
    public function delete(Request $request,$id){
        ...
        $model=DB::table('products')->whereRaw('id="'.$id.'" '.$condition.'')->delete();
    ...
    function skuCheck(Request $request)
    {
		 if($request->post('sku'))
		 {
		  $sku = $request->post('sku');
		  $data = DB::table("products_attr")
		   ->where('sku',$sku )
          // ->where('sku', 'like', $sku . '%')
		   ->count();
		  if($data > 0)
   ...
   public function filePdf() {
                    ...
                    $result['data']= DB::table('products')->whereRaw('(status=1 or status=0) '.$condition.'')->orderBy('id','DESC')->get();
                        ...
                        $pdf = PDF::loadView('admin/product',$result)->setOptions(['defaultFont' => 'sans-serif']);

...
class OrderController extends Controller
{
    ...
    public function order_vendor()
    {
        $result['orders_details']=
                DB::table('orders_details')
                ->select('orders.*','orders_details.qty','products.name as pname','orders_status.orders_status')
                ->leftJoin('orders','orders.id','=','orders_details.orders_id')
                ->leftJoin('products','products.id','=','orders_details.product_id')
                ->leftJoin('orders_status','orders_status.id','=','orders.order_status')
                ->where(['products.added_by'=>session()->get('ADMIN_ID')])
                ->get();   
         return view('admin.order_vendor',$result);
    } 
    ...
    public function exportCSV(Request $request)
{
   ...
   $headers = array(
     ...
     DB::table("orders")->orderBy('id')->chunk(10, function ($data) use ($handle) {
    foreach ($data as $row) {

...
function funSearch(){
  ...
  if(search_str!='' && search_str.length>3){
    window.location.href=URL+'/search/'+search_str;
  }
}
...
jQuery('#frmProductReview').submit(function(e){
  ...
  e.preventDefault();
  jQuery.ajax({
    url:URL+'/product_review_process',
...
$('#emailId').keyup(function(){
		...
		success:function(result)
			{
			 if(result == 'unique')
			 {
			  $('#email_error').html('<label class="text-success">Email Id Not Available in Database</label>');
			  $('#emailId').removeClass('has-error');
			  }
			 else
			 {
			  $('#email_error').html('<label class="text-danger">Email Id already in Database</label>');
	...
	function addWishList(id,size_str_id,color_str_id,uid=''){
	  ...
	  if(size_id=='' && size_id!='no'){
		...
		jQuery.ajax({
		  ...
		  success:function(result){
			...
			alert('Product '+result.msg);
			if(result.totalItem==0){
            jQuery('.aa-cart-notify1').html('0'); 
           }
		   else{
			   jQuery('.aa-cart-notify1').html(result.totalItem);
		   }
        }
  ...
  function deleteWishListProduct(pid,size,color,attr_id,uid=''){
	  
  jQuery('#color_id').val(color);
  jQuery('#size_id').val(size);
  jQuery('#qty').val(0)
  addWishList(pid,size,color,uid);
  //jQuery('#total_price_'+attr_id).html('Rs '+qty*price);
  jQuery('#cart_box1'+attr_id).hide();
}
function addWishList1(id,size_str_id,color_str_id,uid=''){
  jQuery('#color_id').val(color_str_id);
  jQuery('#size_id').val(size_str_id);
  addWishList(id,size_str_id,color_str_id,uid);
}
function updateQtyWishlist(pid,size,color,attr_id,price,uid=''){
  jQuery('#color_id').val(color);
  jQuery('#size_id').val(size);
  var qty=jQuery('#qty'+attr_id).val();
  jQuery('#qty').val(qty)
